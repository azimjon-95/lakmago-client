...
export function AdminPage() {
  ...
  const [tab, setTab] = useState<'main' | 'small'>('main');
  const [editing, setEditing] = useState<Banner | SmallBanner | 'new' | null>(null);

  function saveBanner(data: Omit<Banner, 'id'>, id?: string) {
    if (id) updateBanner(id, data);
    else addBanner(data);
    setEditing(null);
  }
  function saveSmallBanner(data: Omit<SmallBanner, 'id'>, id?: string) {
    if (id) updateSmallBanner(id, data);
    else addSmallBanner(data);
    setEditing(null);
  }
  ...
  if (editing) {
    return (
      <BannerEditor
        ...
        onSave={(data, id) => (tab === 'main' ? saveBanner(data as Omit<Banner, 'id'>, id) : saveSmallBanner(data as Omit<SmallBanner, 'id'>, id))}
      />
    );
  }
...
function BannerEditor({
  ...
  banner: Banner | SmallBanner | null;
  isSmall: boolean;
  onCancel: () => void;
  onSave: (data: Omit<Banner, 'id'> | Omit<SmallBanner, 'id'>, id?: string) => void;
}) {
  const [eyebrow, setEyebrow] = useState(banner?.eyebrow || (isSmall ? 'REKLAMA · SPONSOR' : 'AKSIYA'));
  const [title, setTitle] = useState(banner?.title || '');
  const [cta, setCta] = useState((banner as Banner)?.cta || "Ko'rish");
